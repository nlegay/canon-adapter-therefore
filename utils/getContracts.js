...
exports.getContractsByProductAndCustomer = async (product, customer) => {
    ...
    if ( contractWithCustomerUnique.contracts.length > 0 ) {
        logger.info('getContractsByProductAndCustomer - Find unique customer: ' + contractWithCustomerUnique.contracts[0]["Raison Sociale"]);
...
exports.getContractsByCustomer = async (customer) => {
    ...
    const contractWithCustomerUnique = await getContracts(body);
    if ( contractWithCustomerUnique.contracts.length > 0 ) {
        logger.info('getContractsByCustomer - Find unique customer: ' + contractWithCustomerUnique.contracts[0]["Raison Sociale"]);
        return contractWithCustomerUnique;
    } else {
...
async function getContracts(body) {
    ...
    try {
      ...
      return { "contracts" : contracts.map( c => Object.assign(...fields.map((f, i) => ({ [f]: c[i] }))) ) };
    } catch (err) {
