...
async function sendRequest(name) {
    ...
    try {
        ...
        const customers = resp.QueryResult.ResultRows.map( contrat => contrat.IndexValues[0] );
...
async function getCustomersByName(name) {
    ...
    logger.info('getCustomersByName - customerUnique: ' + JSON.stringify(customerUnique));
